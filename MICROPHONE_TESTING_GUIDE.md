...
| Fictional selectors (`.balance-display`, `[data-testid='voice-button']`) | Real selectors in `HomePage.java`: `[data-testid='listening-hold-to-speak-btn']` (ordinary hold-to-speak), `[data-testid='listening-reconnect-btn']` (a **different** button, "Hold to reconnect", shown only during a "Session Ended" screen — confirmed live), `.mobile-scroll div.justify-end .whitespace-pre-line` (user bubble), `.mobile-scroll div.justify-start .whitespace-pre-line` (bot bubble) |
...
**Voice test suites:** `UI7_BalanceInquiryTest` (44 balance queries, `botverification`), `UI8_TransactionHistoryTest` (transaction-history queries, `botverification`), `UI9_LoanInquiryTest` (76 loan queries, `botverification`), `UI10_TransferMoneyTest` (transfer queries, `botverificationTransferMoney`), `UI11_VoiceRegistrationAuthTest` (voice-match auth), `UI12_MultilingualVoiceQueryTest` (`multilingual`) — all extend `BaseVoiceTest` except UI11/UI12. Run with:
```bash
mvn clean test -DtestGroups=botverification
mvn clean test -DtestGroups=botverificationTransferMoney
```

**Account/loan disambiguation:** when a query is ambiguous (e.g. "What is my balance", "Give me my loan details"), the bot asks which account or loan. `BaseVoiceTest.runVoiceQuery()` detects this via a phrasing check (e.g. `isAccountDisambiguation()` for "choose ... account"/"which ... account"/both SAVINGS and CURRENT named; `UI9_LoanInquiryTest.isLoanDisambiguation()` for "which loan"), then speaks back the answer (from the DataProvider's `disambiguationAccount` field) and retries up to 3 times — because `--use-file-for-fake-audio-capture` loops the WAV it already loaded into memory, and an overwritten file isn't always picked up before the next hold-to-speak window closes. UI9 additionally has a *second* disambiguation stage — "What would you like to know about your X loan?" — walked one category at a time in the same conversation (see `UI9_LoanInquiryTest.walkLoanDetailCategories`).

**Session drops mid-query:** the live bot occasionally shows a "Session Ended" screen mid-conversation (confirmed via `502 Bad Gateway` / `SmallWebRTC Connect error` on stage). `HomePage.recoverFromSessionEndedIfPresent()` clicks the *reconnect* button (not the ordinary hold-to-speak one — see the selector table above), waits through "Connecting," then clicks hold-to-speak again. `BaseVoiceTest.runVoiceQuery()` calls this both before and after each turn, and re-asks the same query (up to `MAX_REASK_ATTEMPTS` times) if the bot answers with its generic session-start greeting or stays blank past `BOT_RESPONSE_TIMEOUT_MS` (30s) instead of actually answering — a session drop that self-heals this way never fails the test, but is still counted by `SessionEndedTracker`/`NoResponseTracker` and shown on the dashboard report.
